...
tm_shape(LargenT, is.master = TRUE, projection = "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs") +
  tm_bubbles(col = "nT", border.col = "black", alpha = 0.8, palette = "Greens", title.col = "Tornadoes") +
...
test <- ggplot(NumClusbyYear) +
 ...
 # geom_bar(aes(y = totTorn, x = Year, fill = "blue"), position = "stack", stat = "identity") +
  ...
  labs(x = "Year", y = "Number of Tornadoes") +
  theme_minimal()
test + theme(legend.position = "bottom", 
...
P1 <- P1 + theme(panel.grid.minor = element_blank(), axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
P1
```
...
P2 <- P2 + theme(panel.grid.minor = element_blank(), axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
P2
```
...
P3 <- P3 + theme(panel.grid.minor = element_blank(), axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
P3
```
...
P4 <- P4 + theme(panel.grid.minor = element_blank(), axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
P4
```
...
t.test(x$decade, x$NumClus,  data = x)
