...
```{r}
WeakTorns <- as.data.frame(HistoricalBigDayTornadoes) %>%
  filter(mag == "F1")
...
ggplot(stateBorders) +
  geom_sf() + 
  stat_density2d(aes(x = slon, y = slat, fill = ..level..), alpha = .5,
                 geom = "polygon", data = centroids) 
```
...
ggplot(NumClusbyYear, aes(fill = maxEF, y = count, x = Year)) +
...
Table: Top ten tornado clusters with the highest fatality counts. 

```{r}

```

Figure: Annual # of fatalities from tornadoes with their respective F-scale ratings color coded. 
