...
ggplot(stateBorders) +
  ...
  stat_density2d(aes(x = slon, y = slat, fill = ..level..), alpha = .7, h = 3, 
  ...
  theme_bw() +
  geom_sf(data = centroids, alpha = 0.3) +
  theme(plot.title = element_text(hjust = 0.5))
```

```{r}
ggplot(HistoricalBigDayTornadoes) +
  geom_hex(bins=25, aes(x = slon, y = slat)) +
...
P3 <- ggplot(NumClusbyHour_SON, aes(fill = maxEF, y = count, x = Hour)) +
  ...
  labs(x = "Hour (CST)", y = "Number of Clusters") +
  theme_minimal() +
  ggtitle("September, October, & November")
```
...
P4 <- ggplot(NumClusbyHour_DJF, aes(fill = maxEF, y = count, x = Hour)) +
  ...
  labs(x = "Hour (CST)", y = "Number of Clusters") +
  theme_minimal() +
  ggtitle("December, January, & February")
```
...
```{r}
HistoricalBigDays.sfdfT %>%
  select(cDate, nT, maxEF, GroupDayCas, GroupDayFat) %>%
