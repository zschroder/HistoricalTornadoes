...
heyya <- for (i in 1:dim(SigTor.sfdf)[1]) {
  tz = (SigTor.sfdf$timezone[i]) #tz_name column in #28 
    bonjour <- US_tz %>%
      filter(tz_name == tz) 
    bonjour <-  as.data.frame(bonjour)
    
    TZAbbr <- ifelse(!is.na(SigTor.sfdf$dst[4] == TRUE), bonjour[2,2], bonjour[1,2])
...
```{r}
heyya <- for (i in 1:dim(SigTor.sfdf)[1]) {
  tz = st_drop_geometry(SigTor.sfdf[i,31]) #tz_name column in #28 
    bonjour <- US_tz %>%
