...
**Currently, the data is a numeric value (i.e., 2300). It needs to be converted to a time object (i.e., 23:00:00). First, create a scaled hour to elliviate negative values for 0 hours. Do this by adding 1 hour to the time column for values less than or equal to 2300. Then, convert to the %H:%M:%S time. Now remove that extra hour (-3600 seconds). Then create a TIME column of formatted time of the tornado occurrence.** 
...
```{r}
SigTor <- SigTor.sfdf %>%
  mutate(cDateTime = Time_CST - as.difftime(6, unit = "hours"),
         hour_CST = hour(Time_CST),
         cDate = as.Date(as_datetime(ifelse(hour_CST < 6, (Time_CST - 86400), cDateTime), tz = Sys.timezone()))) %>%
  sf::st_sf()
```
